...
class ClockworkSupport
{
	...
	{
		NINARouter::get('/__clockwork/{id}/extended', '\NINACORE\Helpers\Clockwork\ClockworkController@getExtendedData')
			->where(['id' => '([0-9-]+|latest)']);
		NINARouter::get('/__clockwork/{id}/{direction?}/{count?}', '\NINACORE\Helpers\Clockwork\ClockworkController@getData')
			->where(['id' => '([0-9-]+|latest)', 'direction' => '(next|previous)', 'count' => '\d+']);
		NINARouter::put('/__clockwork/{id}', '\NINACORE\Helpers\Clockwork\ClockworkController@updateData');
		NINARouter::post('/__clockwork/auth', '\NINACORE\Helpers\Clockwork\ClockworkController@authenticate');
	}
}
